...
class distribution_manager():
    ...
    def __init__(self, inputs):
        ...
        # under normality of self.vec_returns this distributes as chi-square with 2 degrees of freedom
        self.jb_stat = None
    ...
    def __str__(self):
        str_self = self.description + ' | size ' + \
            str(self.nb_rows) + '\n' + self.plot_str()
        return str_self
        ...
        if data_type == 'simulation':
            ...
            if dist_name == 'normal':
                ...
                self.description = data_type + ' ' + \
                    dist_name + ' | df = ' + str(degrees_freedom)
    ...
    def compute(self):
        ...
        self.cvar_95 = np.mean(
            self.vec_returns[self.vec_returns <= self.var_95])
    ...
    def plot_str(self):
        ...
        plot_str = 'mean ' + str(np.round(self.mean, nb_decimals))\
            ...
            + ' | percentile 75% ' + \
            str(np.round(self.percentile_75, nb_decimals))
        return plot_str
...
class capm_manager():
    ...
    def load_timeseries(self):
        self.data_table = file_functions.load_synchronised_timeseries(
            ric_x=self.benchmark, ric_y=self.security)
    ...
    def compute(self):
        ...
        self.correlation = np.round(r_value, self.nb_decimals)
        # pct of variance of y explained by x
...
class hedge_manager():
    ...
    def load_betas(self):
        ...
        print('Beta for ' + security + ' vs ' +
              benchmark + ' is ' + str(portfolio_beta))
        print('Beta mnUSD for ' + security + ' vs ' +
              benchmark + ' is ' + str(portfolio_beta_usd))
        ...
        for hedge_security in hedge_securities:
            ...
            capm.compute()
            beta = capm.beta
            print('Beta for hedge[' + str(counter) + '] = ' +
                  hedge_security + ' vs ' + benchmark + ' is ' + str(beta))
            betas[counter] = beta
            counter += 1
    ...
    def compute(self, regularisation=0.0):
        ...
        betas = self.betas
        optimal_result = minimize(fun=file_functions.cost_function_hedge, x0=x, args=(
            self.portfolio_delta, self.portfolio_beta_usd, betas, regularisation))
        self.optimal_hedge = optimal_result.x
        self.hedge_delta = np.sum(self.optimal_hedge)
        self.hedge_beta_usd = np.transpose(
            betas).dot(self.optimal_hedge).item()
    ...
    def compute_exact(self):
        ...
        if dimensions != 2:
            print('------')
            print('Cannot compute exact solution because dimensions = ' +
                  str(dimensions) + ' =/= 2')
            return
        deltas = np.ones([dimensions])
        betas = self.betas
        targets = -np.array([[self.delta_portfolio],
                             [self.beta_portfolio_usd]])
...
class portfolio_manager:
    ...
    def compute_covariance_matrix(self, bool_print=True):
        ...
        for i in range(size):
            ...
            # returns
            temp_mean = np.round(
                self.scale*np.mean(temp_ret), self.nb_decimals)
            vec_returns[i] = temp_mean
            # volatilities
            temp_volatility = np.round(
                np.sqrt(self.scale)*np.std(temp_ret), self.nb_decimals)
    ...
    def compute_portfolio(self, portfolio_type='default', target_return=None):
        ...
        if portfolio_type == 'min-variance':
            ...
            portfolio.variance_explained = self.eigenvalues[0] / sum(
                abs(self.eigenvalues))
            ...
            res = minimize(file_functions.compute_portfolio_variance, x, args=(
                self.covariance_matrix), constraints=cons)
            ...
            # compute optimisation
            res = minimize(file_functions.compute_portfolio_variance, x, args=(
                self.covariance_matrix), constraints=cons, bounds=bnds)
            weights_normalised = res.x

        elif portfolio_type == 'pca':
            portfolio.type = portfolio_type
            portfolio.variance_explained = self.eigenvalues[-1] / sum(
                abs(self.eigenvalues))
        ...
        portfolio.pnl_annual_usd = np.transpose(
            self.returns).dot(weights).item()
        portfolio.return_annual = np.transpose(
            self.returns).dot(weights_normalised).item()
        portfolio.volatility_annual = file_functions.compute_portfolio_volatility(
            weights_normalised, self.covariance_matrix)
        portfolio.volatility_annual_usd = file_functions.compute_portfolio_volatility(
...
class portfolio_item():
    ...
    def summary(self):
        ...
        if not self.volatility_annual_usd == None:
            print('Volatility annual (mn USD): ' +
                  str(self.volatility_annual_usd))
...
class montecarlo_item():
    ...
    def __init__(self, sim_prices, sim_payoffs, strike, call_or_put):
        ...
        self.confidence_interval = self.mean + \
            np.array([-1, 1])*self.confidence_radius
        if call_or_put == 'call':
